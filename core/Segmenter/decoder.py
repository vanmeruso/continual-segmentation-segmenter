...
class MaskLayerNorm(nn.Module):
    def __init__(self, num_class, eps=1e-5, elementwise_affine=True):
        ...
        if self.elementwise_affine:
            self.weight = nn.ParameterList(nn.Parameter(torch.ones(1)) for _ in range(self.num_class))
            self.bias = nn.ParameterList(nn.Parameter(torch.zeros(1)) for _ in range(self.num_class))
...
class MaskTransformer(nn.Module):
    def __init__(
        ...
        self.cls_emb = nn.ParameterList(nn.Parameter(torch.randn(1, 1, d_model)) for _ in range(self.n_cls))
    ...
    def return_feats(self):
        return self.patches, self.cls_seg_feat
