...
def inference(model,img,ori_shape,window_size,window_stride,batch_size ,device,num_class):

    img = resize(img, window_size)

    preds  = img.new_zeros((batch_size, num_class , ori_shape[0], ori_shape[1]))
    count_mat = img.new_zeros((batch_size, 1, ori_shape[0], ori_shape[1]))
    B, C, H, W = img.shape
    

    """ sliding window for batch"""
    ws = window_size
    ...
    for ha, wa in product (h_anchors, w_anchors):
        crop_img = img[:, :, ha : ha + ws, wa : wa + ws]
        with torch.no_grad():
            with amp.autocast(True):
                crop_seg_logit = model.forward(crop_img)
        preds += F.pad(crop_seg_logit,(int(wa), int(preds.shape[3] - (wa+ws)), int(ha), int(preds.shape[2] - (ha+ws))))
        count_mat[:, :, ha : ha + ws, wa : wa + ws] += 1
